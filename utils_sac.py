...
from torchrl.envs.transforms import Resize, GrayScale, ObservationNorm, ToTensorImage
...
class QNetwork(nn.Module):
    def __init__(self, action_dim, device):
        ...
        self.fc1     = nn.Linear(64 + action_dim, 256, device=device)
        self.relu    = nn.ReLU()
        self.fc2     = nn.Linear(256, 1, device=device)

    def forward(self, pixels, action):
        feats = self.backbone(pixels)    # → [B,64,1,1]
        feats = self.flatten(feats)      # → [B,64]
        x     = torch.cat([feats, action], dim=-1)  # → [B,64+action_dim]
...
def env_maker(cfg, device="cpu", from_pixels=True):
    ...
    if lib in ("gym", "gymnasium"):
        ...
        env = DMControlEnv(
            cfg.env.name, cfg.env.task, from_pixels=True, pixels_only=True
        )
        return env
    else:
...
def make_environment(cfg, logger=None):
    ...
    partial = functools.partial(env_maker, cfg=cfg, from_pixels=True)
...
def make_sac_agent(cfg, train_env, eval_env, device):
    ...
    actor_cnn = ConvNet(
        num_cells=[32, 64, 64],
        squeeze_output=True,                              # pool to a flat vector
        aggregator_class=nn.AdaptiveAvgPool2d,            # yields (batch, C, 1, 1)
        aggregator_kwargs={"output_size": (1, 1)},
        device=device,
    )

    actor_net = nn.Sequential(
        actor_cnn,                                        # → TensorDict with "features": (batch, 64,1,1)
        nn.Flatten(start_dim=1),                         # → (batch, 64)
        nn.Linear(64, 256),                               # hidden MLP
        nn.ReLU(),
        nn.Linear(256, 2 * action_dim),                   # loc & scale heads
    )
    ...
    # --- Q‐value net ----------------------------------------------------------
    # same conv backbone (you can share weights if you like, but here we re‐instantiate)
    q_cnn = ConvNet(
        num_cells=[32, 64, 64],
        squeeze_output=True,
        aggregator_class=nn.AdaptiveAvgPool2d,
    ...
    qvalue_net = QNetwork(action_dim, device=device)
